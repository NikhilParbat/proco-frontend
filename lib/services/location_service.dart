...
class LocationService {
  // Pull the backend URL from Config (same backend as everything else, no separate env key needed)
  static String get _baseUrl =>
      Config.socketUrl(); // <-- CHANGED from dotenv.get('BACKEND_API_URL')
  ...
  static Future<LocationResult?> geocodeAddress(String address) async {
    ...
    try {
      final String url =
          '$_baseUrl/api/location/search?q=${Uri.encodeComponent(address)}';
      final response = await http.get(Uri.parse(url));
  ...
  static Future<List<Map<String, dynamic>>> getPlacePredictions(
    ...
    final String url =
        '$_baseUrl/api/location/autocomplete?q=${Uri.encodeComponent(query)}';
